...
protected:
    struct __node {
        ...
        __node* left;
        __node* right;
        __node* parent; // to use in red-black tree
        colors color; // to use in red-black tree

        __node(const T& val) : value(val), left(nullptr), right(nullptr), parent(nullptr), color(red) {}
